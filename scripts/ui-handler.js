...
// ==========================================
// ✨ SISTEMA MAESTRO DE MOBS ✨
// ==========================================

const MOBS_DB = {
    "zombie": { name: "Zombie", hp: 20, drops: "Rotten Flesh, Cloth, Potato", dim: "Underworld" },
    "skeleton": { name: "Skeleton", hp: 20, drops: "Bones, Arrows, Bow", dim: "Underworld" },
    "creeper": { name: "Creeper", hp: 20, drops: "Gunpowder", dim: "Underworld" },
    "spider": { name: "Spider", hp: 16, drops: "String, Spider Eye", dim: "Underworld" },
    "slime": { name: "Slime", hp: 16, drops: "Slimeball", dim: "Underworld" },
    "pig": { name: "Pig", hp: 10, drops: "Raw Porkchop", dim: "Underworld" },
    "cow": { name: "Cow", hp: 10, drops: "Raw Beef, Leather", dim: "Underworld" },
    "chicken": { name: "Chicken", hp: 4, drops: "Raw Chicken, Feathers", dim: "Underworld" },
    "sheep": { name: "Sheep", hp: 8, drops: "Cloth", dim: "Underworld" },
    
    "zombiepigman": { name: "Zombie Pigman", hp: 20, drops: "Gold Nuggets", dim: "Nether" },
    "ghast": { name: "Ghast", hp: 10, drops: "Gunpowder, Ghast Tear", dim: "Nether" },
    "blaze": { name: "Blaze", hp: 20, drops: "Blaze Rod", dim: "Nether" },
    "magmacube": { name: "Magma Cube", hp: 16, drops: "Magma Cream", dim: "Nether" },
    "nethereye": { name: "Nether Eye", hp: 15, drops: "Eye of Ender", dim: "Nether" },
    
    "enderman": { name: "Enderman", hp: 40, drops: "Ender Pearl", dim: "End" },
    "enderdragon": { name: "Ender Dragon", hp: 333, drops: "Dragon Egg, XP", dim: "End" }
};

let currentMobCategory = 'Underworld';
let selectedMobBrowserId = null;
let currentMobToSpawn = 'zombie';
let quickAccessMobs = ['zombie', 'skeleton'];

function openMobModal() {
    document.getElementById('mob-browser-modal').style.display = 'flex';
    renderMobGrid();
}

function switchMobTab(dim, btnElement) {
    currentMobCategory = dim;
    document.querySelectorAll('.mob-tab').forEach(btn => btn.style.background = '#1a1a1a');
    btnElement.style.background = '#333'; // Tab activa
    renderMobGrid();
}
...
function renderMobGrid() {
    ...
    for (const [mobId, data] of Object.entries(MOBS_DB)) {
        if (data.dim !== currentMobCategory) continue;

        let card = document.createElement('div');
        card.style.width = '80px';
        card.style.height = '100px';
        card.style.background = selectedMobBrowserId === mobId ? '#4DA6FF' : '#444';
        ...
        if (img && img.complete && img.naturalWidth > 0) {
            ...
            let scale = Math.min(64 / img.naturalWidth, 64 / img.naturalHeight);
            ...
            placeholder.style.width = '64px'; placeholder.style.height = '64px';
        ...
        card.onclick = () => {
            selectedMobBrowserId = mobId;
            updateMobSidebar();
            renderMobGrid(); // Re-render para iluminar el seleccionado
        };
        grid.appendChild(card);
    }
}

function updateMobSidebar() {
    if (!selectedMobBrowserId) return;
    const data = MOBS_DB[selectedMobBrowserId];

    document.getElementById('mob-preview-name').innerText = data.name;
    document.getElementById('mob-preview-hp').innerText = data.hp + " ❤";
    document.getElementById('mob-preview-drops').innerText = data.drops;

    const btn = document.getElementById('btn-spawn-mob-action');
    btn.disabled = false;
    btn.style.opacity = '1';
    btn.style.cursor = 'pointer';

    // Dibujar el Preview grande
    const cvs = document.getElementById('mob-preview-canvas');
    const ctx = cvs.getContext('2d');
    ctx.clearRect(0, 0, cvs.width, cvs.height);
    let img = window.images[selectedMobBrowserId];
    
    if (img && img.complete && img.naturalWidth > 0) {
        ctx.imageSmoothingEnabled = false;
        let scale = selectedMobBrowserId === 'enderdragon' ? 1 : 4; 
        ...
        ctx.font = "14px Arial";
        ctx.fillText("No Texture", cvs.width/2, cvs.height/2);
    }
}

function confirmMobSpawn() {
    if (!selectedMobBrowserId) return;
    
    currentMobToSpawn = selectedMobBrowserId;
    
    // Actualizar los últimos 3 usados
    if (!quickAccessMobs.includes(currentMobToSpawn)) {
        quickAccessMobs.unshift(currentMobToSpawn); // Agrega al principio
        if (quickAccessMobs.length > 2) quickAccessMobs.pop(); // Elimina el 4to
    }
    
    updateMobQuickAccess();
    closeModal('mob-browser-modal');
    selectTool('spawn_mob'); // Activar la herramienta
}

function updateMobQuickAccess() {
    const container = document.getElementById('recent-mobs-ribbon');
    if (!container) return;
    container.innerHTML = '';
    
    quickAccessMobs.forEach(mobId => {
        let btn = document.createElement('div');
        btn.style.width = '64px';
        btn.style.height = '64px';
        btn.style.background = currentMobToSpawn === mobId ? '#4DA6FF' : '#555';
        btn.style.border = '1px inset #FFF';
        btn.style.cursor = 'pointer';
        btn.style.display = 'flex';
        btn.style.justifyContent = 'center';
        btn.style.alignItems = 'center';
        btn.title = MOBS_DB[mobId] ? MOBS_DB[mobId].name : mobId;
        ...
        if(img && img.complete && img.naturalWidth > 0) {
            ...
            let ctx = cvs.getContext('2d');
            ctx.imageSmoothingEnabled = false;
            let scale = Math.min(48 / img.naturalWidth, 48 / img.naturalHeight);
        ...
        btn.onclick = () => {
            currentMobToSpawn = mobId;
            selectTool('spawn_mob');
            updateMobQuickAccess(); 
        };
        container.appendChild(btn);
    });
}
// Renderizamos la barra rápida al inicio
setTimeout(updateMobQuickAccess, 1000);